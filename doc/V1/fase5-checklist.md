...
- **Gap di correttezza reale trovato in analisi (09/09/2026), non solo teorico**: `text-extraction.js` aveva un fallback silenzioso — qualunque file non PDF/DOCX veniva "estratto" tentando `buffer.toString('utf-8')` sui byte grezzi. Per un'immagine o un PPT questo non lancia errore: produce una stringa spazzatura (bytes binari interpretati come UTF-8), che veniva comunque chunkizzata, embeddata e salvata come se fosse contenuto vero — Billy avrebbe potuto "citare" un documento il cui contenuto reale è rumore casuale.
  **Risolto**: aggiunto `isTxt()` esplicito (solo `.txt`/`text/plain` usa il fallback di decodifica diretta) e un ramo finale che **rifiuta** con errore 400 chiaro qualunque formato non riconosciuto, invece di tentare comunque la decodifica. Verificato in produzione (CF): upload di un file con estensione non supportata (`immagine.png`) → `400 Bad Request` con messaggio `"Formato file non supportato (...). Formati accettati: PDF, DOCX, PPTX, TXT."`, nessun asset creato.
- **Aggiunto supporto PPTX** (libreria `officeparser`, verificata su un `.pptx` reale prima dell'integrazione — stesso metodo già usato per pdf-parse/mammoth in §1.8): `isPptx()` + `officeParser.parseOffice(buffer, { fileType: 'pptx' })` → `ast.to('text')`. Verificato in produzione (CF): upload `.pptx` reale (2MB, slide di business review) → approvazione → Billy cita correttamente contenuti specifici delle slide (non testo spazzatura). Frontend (`app/index.html`, `upload-view.js`) aggiornato di conseguenza (accetta `.pptx`, messaggi coerenti).
- **Bug di build scoperto durante il test**: `mbt build` riusa la cartella `gen/` (gitignored, mai pulita in automatico) se già presente da un build precedente — il modulo `billy-srv` risultava pacchettizzato con un `package.json` non aggiornato (mancava `officeparser`), scoperto controllando il contenuto reale del `.mtar` (il pacchetto MTA non include comunque `node_modules`: lo installa il buildpack CF in staging da `package.json`/`package-lock.json`, quindi il problema era proprio il `package.json` copiato, non i moduli mancanti). **Fix**: `rm -rf gen mta_archives .billy-app_mta_build_tmp` prima di ogni `mbt build`, non solo `mta_archives`/`.billy-app_mta_build_tmp` come fatto finora.
